...
import { init, donate, get_donation_by_number } from '..';
import { get_beneficiary, get_number_of_donation } from '../model';
...
describe("Donations", () => {
  it("stores and retrieves donations", ()=>{
    ...
    expect(first_donation.account_id).toBe("donor_a")
    ...
    expect(second_donation.account_id).toBe("donor_b")
    expect(second_donation.amount).toBe(u128.from(2) * NEAR)

    expect(get_number_of_donation()).toBe(2)
  })
})
